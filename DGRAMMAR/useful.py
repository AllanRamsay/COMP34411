...
def usingMac():
    return "Allan-Ramsays-MacBook-Pro" in host or host == "armac.cs.man.ac.uk"
...
class safeout:
    ...
    def __init__(self, x, mode='w', encoding=False):
        ...
        if isstring(x):
            if encoding:
                self.writer = codecs.open(f, mode, encoding)
            else:
...
def timediff(t1, t0=False):
    if t0 == False:
        t1, t0 = now(), t1
    t1 = (t1-t0)
    return (t1.seconds*1000000+t1.microseconds)/1000000.0
