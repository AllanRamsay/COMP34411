...
def framesig(sig,frame_len,frame_step,winfunc=lambda x:numpy.ones((x,))):
    ...
    print "frame_len %s, numframes %s, frame_step %s"%(frame_len, numframes, frame_step)
...
def preemphasis(signal,coeff=0.95):
    ...
    If the change between signal[i] and signal[i-1] is large, then signal[i]-0.95*signal[i-1]
    will be large. And the gap between signal[i] and signal[i-1] will be large if the gradient
    at i is large. If you're on a steep slope with peak P then the gradient will be larger than
    if you're on a gentle slope where the peak is P, i.e. rapidly changing (=high frequency)
    signals with the same amplitude will be emphasised. THEY WILL SOUND PRETTY WEIRD.
    """    
