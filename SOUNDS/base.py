...
def mfcc(signal,samplerate=16000,winlen=0.025,winstep=0.01,numcep=13,
    ...
    :returns: A pylab array of size (NUMFRAMES by numcep) containing features. Each row holds 1 feature vector.
    """            
...
def matchframes(frame0, frame1):
    frame0 = frame0.tolist()
    frame1 = frame1.tolist()
    while len(frame0) > 0:
        if frame0 == frame1[:len(frame0)]:
            print "FOUND IT %s"%(len(frame0))
        frame0 = frame0[1:]
...
def lifter(cepstra,L=22):
    ...
    """
    if L > 0:
        nframes,ncoeff = pylab.shape(cepstra)
        n = pylab.arange(ncoeff)
        lift = 1+ (L/2)*pylab.sin(pylab.pi*n/L)
        return lift*cepstra
    else:
